...
def build_outputs(df: pd.DataFrame) -> None:
    ...
    if "срок" in worst_product_issue:
        worst_product_action = "внедрить обязательную проверку срока поставки перед подтверждением заказа в ERP"
    else:
        worst_product_action = f"внедрить обязательную проверку причины '{worst_product_issue}' перед подтверждением заказа в ERP"
    revenue_priority = (
...
1. Для `{worst_product}`: {worst_product_action}.
2. Ввести контроль первого ответа по почтовому каналу.
3. Для `{revenue_priority}` подготовить шаблон ответа по причине `{revenue_priority_issue}` и обновить описание условий в коммерческом предложении.
...
def build_readme(df: pd.DataFrame) -> None:
    ...
    (PROJECT / "README.md").write_text(
        f"""# Customer Experience Analytics
...
- для `{worst_product}`: {worst_product_action};
- ввести SLA на первый ответ по почтовому каналу;
- для `{revenue_priority}` подготовить шаблон ответа по причине `{revenue_priority_issue}` и обновить описание условий в коммерческом предложении;
